...
function assertHandle() {
  if (!HANDLE) throw new Error("InfinitePay ainda não configurada para a rifa do Júnior.");
}

function assertPersonalPix() {
  if (!PERSONAL_PIX_KEY || !PERSONAL_PIX_OWNER) {
    throw new Error("Pix pessoal ainda não configurado para a rifa do Júnior.");
  }
}
...
function normalizeBrazilPhone(value: unknown, requireMobile = false): string {
  ...
  if (!d) throw new Error("Telefone não informado.");
  ...
  if (d.length !== 10 && d.length !== 11) throw new Error("Telefone inválido. Use DDD + número.");
  const ddd = d.slice(0, 2);
  const subscriber = d.slice(2);
  if (!validDdds.has(ddd)) throw new Error(`DDD ${ddd} inválido.`);
  if (d.length === 11 && subscriber[0] !== "9") throw new Error("Celular inválido: depois do DDD, o número deve começar com 9.");
  if (d.length === 10 && requireMobile) throw new Error("Para WhatsApp de celular, informe DDD + 9 dígitos.");
  if (d.length === 10 && !/^[2-9]/.test(subscriber)) throw new Error("Telefone fixo inválido.");
  return d;
}
...
async function createPurchase(body: any) {
  ...
  const { data: started, error: startError } = await db.rpc("junior_start_infinitepay_payment", {
    p_name: name,
    p_whatsapp: whatsapp,
    p_numbers: numbers,
  });
  ...
  if (!Number.isInteger(unitPrice) || unitPrice <= 0) {
    ...
    throw new Error("Valor unitário inválido no estado da rifa.");
  }
  ...
  try {
    ...
    const response = await fetch("https://api.checkout.infinitepay.io/links", {
      ...
      body: JSON.stringify({
        ...
        items: [{
          ...
          price: unitPrice,
          description: `Rifa Beneficente Júnior - ${selectedNumbers.length} número(s)`,
        }],
      }),
    });
    ...
    await db.from("junior_reservations").update({
...
async function createPersonalPix(body: any) {
  ...
  const { data, error } = await db.rpc("junior_start_personal_pix_payment", {
    p_name: name,
    p_whatsapp: whatsapp,
    p_numbers: numbers,
  });
...
async function createCashPayment(body: any) {
  ...
  const { data, error } = await db.rpc("junior_start_cash_payment", {
    ...
    p_whatsapp: whatsapp,
    p_numbers: numbers,
    p_cash_received_by: receivedBy,
    p_cash_received_phone: receivedPhone,
  });
  if (error) throw error;
  return data;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body?.action === "create") return json(await createPurchase(body));
    if (body?.action === "create_personal_pix") return json(await createPersonalPix(body));
    if (body?.action === "personal_pix_contacted") return json(await markPersonalPixContacted(String(body.order_nsu || "")));
    if (body?.action === "create_cash_payment") return json(await createCashPayment(body));

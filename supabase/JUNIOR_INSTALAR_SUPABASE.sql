-- ================================================================
-- RIFA BENEFICENTE — CIRURGIA DO JÚNIOR
-- INSTALAÇÃO INDEPENDENTE NO MESMO PROJETO SUPABASE
-- ================================================================
-- Cria SOMENTE objetos com prefixo junior_ / private.junior_.
-- Não altera tabelas ou dados de outras rifas existentes.
-- Meta: R$ 10.000,00 | quantidade configurável pelo Admin | R$ 10,00 por número.
-- ================================================================
...
values (1,false,null,null,null,1000000,1000,1000)
...
-- O PIN de 4 dígitos das ações sensíveis continua separado e deve ser definido depois.
...
-- PIN administrativo começa bloqueado até você definir os 4 dígitos desejados.
insert into private.junior_admin_action_pin(id,pin_sha256)
values (1,encode(extensions.digest(extensions.gen_random_uuid()::text,'sha256'),'hex'))
on conflict (id) do nothing;

-- Encerra sessões antigas para garantir que o novo login entre limpo.
...
begin
  ...
  return jsonb_build_object(
    'stats',jsonb_build_object(
      'available',(select count(*) from public.junior_raffle_numbers where status='available'),
      'pending',(select count(*) from public.junior_raffle_numbers where status='pending'),
      'paid',(select count(*) from public.junior_raffle_numbers where status='paid'),
...
-- FIM DA INSTALAÇÃO PRINCIPAL.
-- LOGIN JÁ CONFIGURADO: junior@gmail.com / senha 123456.
-- PENDENTE SOMENTE: definir o PIN de 4 dígitos das ações administrativas.
